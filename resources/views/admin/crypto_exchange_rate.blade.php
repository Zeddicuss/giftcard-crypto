...
<div class="col-lg-12">
    <div class="card">
        <div class="card-header">
            <h4 class="card-title">Crypto Exchange Rates</h4>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <div><button type="button" class="btn btn-outline-secondary btn-xs" data-bs-toggle="modal"
                    data-bs-target="#setCrypto"><span class="btn-icon-start text-secondary"><i
                        class="fa fa-plus"></i>
                        </span>Set Cypto Exchange</button>
                </div>
                <table class="table table-responsive-md">
                    <thead>
                        <tr>
                            ...
                            <th><strong>Exchange Crypto</strong></th>
                            <th><strong>SYMBOL</strong></th>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <span class="w-space-no">{{$rate->crypto}}</span>
                                </div>
                            </td>
                            <td>{{$rate->symbol}}</td>
                            <td>{{$rate->exchange_rate}}</td>
                            <td>{{$rate->exchange_crypto}}</td>
                            <td>{{$rate->exchange_symbol}}</td>
                            <td>
                                <div class="d-flex">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Set Crypto Exchange</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
            
                    <form method="POST" action="{{route('crypto.set')}}" enctype="multipart/form-data">
                        @csrf
                        <div class="modal-body">
                            <label class="form-label d-block mt-3">Crypto Name</label>
                            <input type="text" name="crypto" class="form-control w-100" placeholder="Eg Bitcoin, Etherum">
                            @error('crypto')
                                <div>{{ $message }}</div>
                            @enderror
                            ...
                            <label class="form-label d-block mt-3">Exchange Crypto</label>
                            <input type="text" name="exchange_crypto" class="form-control w-100" placeholder="Eg Etherum, Dodge Coin">
                            @error('exchange_crypto')
                                <div>{{ $message }}</div>
                            @enderror

                            
                            <label class="form-label d-block mt-3">Exchange Symbol</label>
                            <input type="text" name="exchange_symbol" class="form-control w-100" placeholder="Eg BTC, ETH">
            ...
            <div class="modal fade" id="editCrypto" tabindex="-1" aria-labelledby="editProfileLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Edit Crypto Exchange</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
